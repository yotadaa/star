...
const ARTICLE_SEO_OVERRIDES = Object.freeze({
...
export function buildBlogArticleSeo(post = {}) {
  ...
  const seoTitle = String(seoOverride?.seoTitle || post.seoTitle || post.title || "").trim();
  const seoDescription = String(seoOverride?.seoDescription || post.seoDescription || post.excerpt || "").trim();
  ...
  const author = {
    "@type": "Person",
    "@id": personId,
    name: displayOwnerText(post.author?.name || profile.name).trim(),
    url: authorUrl,
  };
