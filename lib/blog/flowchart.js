...
export function parseBlogFlowchart(source) {
  ...
  if (input.length > MAX_SOURCE_LENGTH) return { ok: false, reason: "Flowchart melebihi batas sumber." };
  const lines = input
    .replace(/\r\n/g, "\n")
    .split("\n")
    .map((line) => line.trim())
    .filter((line) => line && !line.startsWith("%%"));
  if (lines.length > MAX_LINES) return { ok: false, reason: "Flowchart memiliki terlalu banyak baris." };
  const header = lines.shift()?.match(HEADER_PATTERN);
  if (!header) return { ok: false, reason: "Header flowchart tidak dikenali." };
  ...
  for (const line of lines) {
    ...
    if (!edge) return { ok: false, reason: `Sintaks belum didukung: ${line}` };
    const from = parseNode(edge[1]);
    const to = parseNode(edge[3]);
    if (!from || !to) return { ok: false, reason: `Node belum didukung: ${line}` };
    register(from);
    register(to);
    edges.push({ from: from.id, to: to.id, type: edge[2] });
    if (nodeMap.size > MAX_NODES || edges.length > MAX_EDGES) {
      return { ok: false, reason: "Flowchart melebihi batas node atau relasi." };
    }
  }

  if (!edges.length || !nodeMap.size) return { ok: false, reason: "Flowchart tidak memiliki relasi yang dapat dirender." };
