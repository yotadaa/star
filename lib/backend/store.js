...
export async function healthCheckBackend() {
  const status = await fetchAction(api.bridge.health, { secret: bridgeSecret() });
...
export async function createRecord({ collection, payload, visibility, slug, actor }) {
  return await fetchAction(api.bridge.createRecord, {
    secret: bridgeSecret(),
...
export async function listRecords({ collection, limit, includePrivate = false } = {}) {
  ...
  if (includePrivate) {
    return await fetchAction(api.bridge.listRecordsAdmin, { secret: bridgeSecret(), ...args });
  }
  return await fetchQuery(api.records.listPublic, args);
}

export async function getRecordById(id) {
  return await fetchAction(api.bridge.getRecordAdmin, { secret: bridgeSecret(), id });
}

export async function deleteRecordById(id, actor) {
  return await fetchAction(api.bridge.removeRecord, {
    secret: bridgeSecret(),
    id,
    actor: actorSnapshot(actor),
  });
...
export async function uploadFile({ file, recordId, metadata = {}, actor }) {
  const actorData = actorSnapshot(actor);
  const uploadUrl = await fetchAction(api.bridge.createFileUploadUrl, {
    secret: bridgeSecret(),
    actor: actorData,
  });
  ...
  const fileId = await fetchAction(api.bridge.commitFile, {
    secret: bridgeSecret(),
    storageId,
...
export async function getFileById(id) {
  return await fetchAction(api.bridge.getFile, { secret: bridgeSecret(), id });
}
