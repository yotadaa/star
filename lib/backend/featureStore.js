...
export async function deleteChatMessage({ id, actor }) {
  ...
  try {
    return await fetchAction(api.bridge.deleteWorldChat, {
      secret: bridgeSecret(),
      id: messageId,
      actor: actorSnapshot(actor),
    });
...
export async function getNalaSettings() {
  return await fetchAction(api.bridge.getNalaSettings, {
    secret: bridgeSecret(),
  });
}
...
export async function updateNalaSettings({ payload, actor }) {
  ...
  return await fetchAction(api.bridge.updateNalaSettings, {
    secret: bridgeSecret(),
    payload: {
