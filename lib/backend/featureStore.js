...
const ENGLISH_CAPTIONS = {
  "about-caption": `${profile.affiliation} · ${profile.location}`,
  "projects-caption": "Selected work across web development, AI, and data science. Filter by type or category, or browse all 57 repositories on GitHub.",
  "research-caption": "Published, indexed research with verified Google Scholar citations.",
  "blog-caption": "Notes on research, web builds, and community work, stored and synchronized through Convex.",
  "contact-caption": "Have a project, a research question, or simply want to say hello? Choose the channel that suits you.",
};
...
function englishAboutEntry(entry) {
  if (entry.entryKey === "intro") {
    ...
    return {
      ...
      body: bodyLooksIndonesian ? fallback.body : entry.body,
      payload: {
        ...entry.payload,
  ...
  if (ENGLISH_CAPTIONS[entry.entryKey]) {
    return { ...entry, body: ENGLISH_CAPTIONS[entry.entryKey] };
  }
  return entry;
}
...
function blogPostSummary(post) {
  const featuredImage = post.featuredImage
    || (Array.isArray(post.blocks) ? post.blocks.find((block) => block?.type === "image") : undefined);
