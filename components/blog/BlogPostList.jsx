...
export default function BlogPostList({ posts, canManageBlog = false }) {
  ...
  const tags = useMemo(() => ["Semua", ...Array.from(new Set(items.flatMap((post) => post.tags || [])))], [items]);
  const [activeTag, setActiveTag] = useState("Semua");
  const [view, setView] = useState("list");
  const visiblePosts = activeTag === "Semua" ? items : items.filter((post) => (post.tags || []).includes(activeTag));

  async function archivePost(post) {
    setActionState(`Mengarsipkan ${post.title}...`);
    try {
      ...
      setActionState(`${post.title} diarsipkan`);
    } catch (error) {
  ...
  return (
    <section className="blog-list-section" aria-label="Lore entries">
      <div className="blog-toolbar">
        <div className="cat-filters" role="tablist" aria-label="Filter kategori blog">
          {tags.map((tag) => (
            <PixelButton
              key={tag}
        ...
        <div className="toolbar-right">
          ...
          <div className="blog-view-toggle" aria-label="Ganti tampilan blog">
            <button
              type="button"
