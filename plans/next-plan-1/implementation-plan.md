...
- Placeholder input: `"Ketik pesan ke semua penjelajah..."` — bahasa RPG
  ...
  tidak norak).
- Indikator online count kecil di header panel memakai `icon-marker-current`
  atau dot SVG monoline: `"3 penjelajah online"` dengan warna token status
  dari design system. Angka ini **harus** angka asli dari jumlah koneksi
  realtime aktif, bukan angka dikarang (Supabase Realtime Presence API bisa
  hitung ini).
...
tombol "Tambah Item" dengan `icon-admin-shield` muncul (hanya untuk role owner) →
...
### 3.5 UI
- Tombol "Tambah Item" dengan ikon SVG **hanya render kalau `role === 'owner'`** —
...
- Tombol "Tulis Baru" dengan `icon-editor-blocks` → `/blog/admin/new`.
...
│ [Judul post...]                    [Publish]│
│ ───────────────────────────────────────────│
│  [H1] [H2] [B] [I] [Quote] [Code] [Image]   │ ← toolbar block
│ ───────────────────────────────────────────│
│                                              │
│   (area edit Tiptap, WYSIWYG block-based)   │
│                                              │
│ ───────────────────────────────────────────│
│ Slug: auto-dari-judul   Tags: [+]   Cover:[↑]│
│ Status: ● Draft  ○ Published                │
└───────────────────────────────────────────┘
```

- Toolbar floating saat teks di-select (bubble menu Tiptap) untuk format
  cepat (bold/italic/link), plus toolbar block tetap di atas untuk insert
  block baru — pola umum editor block modern (Notion-like), sesuai
  permintaan "block level editor".
- Indikator "Tersimpan otomatis · 2 detik lalu" di pojok, reuse pola
  Toast tapi versi inline-text (bukan toast popup, supaya tidak
  mengganggu saat sedang menulis).
...
| `BlockEditor.jsx` | wrapper Tiptap + toolbar block + bubble menu |
| `EditorToolbar.jsx` | toolbar insert block |
...
```
[status MB · NST]  Home  About  Projects  Research  Blog  Contact  [icon-compass][icon-backpack][icon-chat-bubble][icon-command]  [Avatar/Login]
```

- **Blog** ditambah sebagai nav item baru sejajar Home/About/dst (bukan
  ikon — ini halaman utama, bukan utility popup).
- **`icon-backpack`** — trigger popup Inventory/Achievement/Mission (dari
  `popup-system-design.md`).
- **`icon-chat-bubble`** — trigger panel World Chat (baru, item ini).
...
- Mobile: item nav ketiga ke atas (About–Blog) masuk ke hamburger/overflow
  menu, ikon utility dari SVG sprite tetap tampil sebagai row terpisah,
  avatar tetap paling kanan.
